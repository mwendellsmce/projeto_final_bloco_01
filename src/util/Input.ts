...
export class Input {
    ...
    /**
     * Faz uma pergunta de Sim/Não ao usuário (Y/N)
     * * USO: Para CONFIRMAÇÕES (ex: "Deseja realmente excluir?")
     * RETORNO: true (Sim) ou false (Não)
     */
    static keyInYN(pergunta: string): boolean {
        const readlinesync = require('readline-sync');
        return readlinesync.keyInYN(pergunta);
    }
}
